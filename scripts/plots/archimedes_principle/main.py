def plot_archimedes_principle(object_density, fluid_density=1000, object_volume=1):
    ...
    ax.add_patch(
        patches.Rectangle((1.5, object_position), 1, object_height, color="brown")
    )

    # Draw forces
    ax.annotate(
        "",
        xy=(2, object_position + object_height),
        xytext=(2, object_position + object_height + 1),
    ...
    ax.annotate(
        "",
        xy=(2, object_position),
        xytext=(2, object_position - 1),
        arrowprops=dict(arrowstyle="->", lw=2),
        annotation_clip=False,
    )
    ...
    if floats:
        ax.text(
            0.5,
            1.5,
            "The object floats because the buoyant force >= weight.",
            fontsize=12,
            color="green",
        )
    else:
...
# Example usage
plot_archimedes_principle(
    object_density=500
)  # Adjust density to see different behaviors
