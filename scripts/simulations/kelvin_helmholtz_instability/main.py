...
for j in range(temp_width):
    ...
    for i in range(temp_height):
        ...
        u[i, j] = perturbation_amplitude * random_factor * np.sin(frequency * np.pi * adjusted_i / temp_height) * np.cos(frequency * np.pi * j / temp_width)
        v[i, j] = perturbation_amplitude * random_factor * np.cos(frequency * np.pi * adjusted_i / temp_height) * np.sin(frequency * np.pi * j / temp_width)
...
def temperature_to_color(temp, min_temp, max_temp):
    norm_temp = (temp - min_temp) / (max_temp - min_temp) if max_temp > min_temp else np.zeros_like(temp)
...
def advect(field, u, v):
    ...
    field_new[1:-1, 1:-1] = (
        s0[1:-1, 1:-1] * (t0[1:-1, 1:-1] * field[i0[1:-1, 1:-1], j0[1:-1, 1:-1]] + t1[1:-1, 1:-1] * field[i1[1:-1, 1:-1], j0[1:-1, 1:-1]]) +
        s1[1:-1, 1:-1] * (t0[1:-1, 1:-1] * field[i0[1:-1, 1:-1], j1[1:-1, 1:-1]] + t1[1:-1, 1:-1] * field[i1[1:-1, 1:-1], j1[1:-1, 1:-1]])
    )
...
while running:
    ...
    for i in range(temp_height):
        for j in range(temp_width):
            ...
            if draw_grid:
                pygame.draw.rect(screen, (255, 255, 255), pygame.Rect(j * cell_width, i * cell_height, cell_width, cell_height), 1)  # Draw grid lines
