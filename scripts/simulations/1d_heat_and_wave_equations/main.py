...
if alpha_heat >= alpha_heat_max:
    print(
        f"Warning: The heat equation solver may be unstable (alpha_heat = {alpha_heat:.2f} >= 0.5)."
    )
if alpha_wave >= alpha_wave_max:
    print(
        f"Warning: The wave equation solver may be unstable (Courant number = {np.sqrt(alpha_wave):.2f} >= 1)."
    )
...
ani = animation.FuncAnimation(
    fig, animate, frames=Nt, interval=20, blit=True, repeat=False
)
